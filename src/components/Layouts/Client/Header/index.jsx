...
const Header = () => {
  ...
  const fetchApi = async () => {
    try {
      if(currentUser!= null){
        console.log("first")
        const result = await axios.get(`https://spring-api-w8iy.onrender.com/api/patient/${currentUser.user.id}`);
        setUser(result.data);
      }
    } catch (error) {
    }
            ...
            <motion.div>
              {currentUser != null && currentUser.user.roles[0] == 'USER' ? (
                <div className='icon_login'>
                  <div>
                    {Object.keys(user).length != 0 && user.image != ''?<img src={"https://spring-api-w8iy.onrender.com/images/patients/" + user.image} alt="" id='user-login' onClick={handleMenuToggle} className='img__icon_login img-fluid' />:
