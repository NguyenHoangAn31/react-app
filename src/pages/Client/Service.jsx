...
const Service = () => {
  ...
  useEffect(() => {
    const loadService = async () => {
      try {
        const response = await axios.get('https://spring-api-w8iy.onrender.com/api/department/all');
  ...
  useEffect(() => {
    if (activeServiceIndex !== null) {
      const loadDoctors = async () => {
        try {
          const response = await axios.get(`https://spring-api-w8iy.onrender.com/api/doctor/related-doctor/${activeServiceIndex}`);
