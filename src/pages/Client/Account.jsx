...
const Account = () => {
  ...
  const fetchDataPatient = async () => {
    try {
      if (currentUser?.user.id) {
        try {
          const response = await axios.get(`https://spring-api-w8iy.onrender.com/api/patient/getpatientdetail/${currentUser?.user.id}`);
  ...
  const handleFormSubmit = async () => {
    try {
      ...
      await axios.put(`https://spring-api-w8iy.onrender.com/api/patient/editpatient/${currentUser?.user.id}`, formData);
      alert("Changes successfully");
      fetchDataPatient();
      const checkToken = await axios.post('https://spring-api-w8iy.onrender.com/api/auth/check-refresh-token', {
  ...
  const handleCreate = async () => {
    try {
      ...
      await axios.post(`https://spring-api-w8iy.onrender.com/api/patient/create/${currentUser?.user.id}`, formData);
      fetchDataPatient();
      const checkToken = await axios.post('https://spring-api-w8iy.onrender.com/api/auth/check-refresh-token', {
        username: currentUser.user.phone,
        provider: "phone"
    ...
    <>
      ...
      <div className='d-flex gap-5' style={{ marginBottom: 125 }}>
        {isAccount ? <div className='w-50 text-center mt-2'>
          <img src={`https://spring-api-w8iy.onrender.com/images/patients/${patient.image}`} alt="" className='img-fluid' />
